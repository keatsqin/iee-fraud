"""
方案二：特征级融合运行脚本
流程：加载图数据 -> 训练GNN -> 社群检测 -> 融合特征训练LightGBM -> 评估对比

运行方式（从项目根目录）:
    python -m src.models.run_community_feature_fusion
或:
    python src/models/run_community_feature_fusion.py
"""
import sys
import json
import numpy as np
...
from src.models.community_feature_fusion import CommunityFeatureFusionDetector
...
OUTPUT_DIR  = PROJECT_ROOT / "outputs" / "community_fusion"
...
def train_and_evaluate(data, embeddings, community_detector,
    ...
    fusion.train(data, embeddings, train_mask, val_mask,
                 num_boost_round=2000, early_stopping_rounds=100)
...
def save_results(results: dict, baseline: HybridFraudDetector,
    ...
    # 模型文件
    baseline.save(str(OUTPUT_DIR / "baseline_hybrid.pkl"))
    fusion.save(str(OUTPUT_DIR / "fusion_hybrid.pkl"))

    # 特征重要性 CSV
    imp = fusion.get_feature_importance(top_k=50)
    imp.to_csv(OUTPUT_DIR / "fusion_feature_importance.csv", index=False)
