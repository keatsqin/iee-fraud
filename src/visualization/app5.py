...
DOCX_IMBALANCE_ROWS = [
    ...
    {'model': '基线模型 + 社群特征', 'auc': 0.98861, 'ap': 0.91381, 'f1': 0.85967, 'precision': 0.90932, 'recall': 0.81516},
]
...
def precompute_stats(edges_df=None):
    ...
    # 加载原始数据
    try:
        train_transaction = pd.read_csv('D:/ZM/53453123/ieee-fraud-detection/train_transaction.csv')
        df = train_transaction.copy()
        logger.info(f"Loaded {len(df)} transactions")

        # 尝试加载identity数据以获取更多特征
        try:
            train_identity = pd.read_csv('D:/ZM/53453123/ieee-fraud-detection/train_identity.csv')
    ...
    return html.Div([
        ...
        html.Div([
            html.Div([
                ...
                dcc.Graph(id='docx-four-models', style={'height': '350px'}, config={'displayModeBar': False})
            ], style={'flex': '1', 'backgroundColor': COLORS['card'], 'padding': '15px', 'borderRadius': '10px'}),
        ], style={'display': 'flex', 'gap': '20px', 'marginBottom': '20px'}),
        ...
        html.Div([
            html.Div([
                html.H3("⚖️ 社群特征加入 vs 基线", style={'color': COLORS['text'], 'marginBottom': '10px', 'fontSize': '16px'}),
                dcc.Graph(id='docx-imbalance-compare', style={'height': '350px'}, config={'displayModeBar': False})
            ], style={'flex': '1', 'backgroundColor': COLORS['card'], 'padding': '15px', 'borderRadius': '10px'}),
            html.Div([
                html.H3("🔑 特征重要性TOP20", style={'color': COLORS['text'], 'marginBottom': '10px', 'fontSize': '16px'}),
                dcc.Graph(id='feature-importance', style={'height': '350px'}, config={'displayModeBar': False})
            ], style={'flex': '1', 'backgroundColor': COLORS['card'], 'padding': '15px', 'borderRadius': '10px'}),
        ], style={'display': 'flex', 'gap': '20px'}),
    ], style={'padding': '20px'})
...
def update_feature_importance(_):
    if DATA.get('model_results') and 'feature_importance' in DATA['model_results']:
        ...
        # 默认特征重要性数据
        features = [f'feature_{i}' for i in range(20, 0, -1)]
