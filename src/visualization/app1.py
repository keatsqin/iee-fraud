...
DOCX_HYBRID_ENHANCE_ROWS = [
    {'model': 'GraphSAGE + LightGBM (基线)', 'auc': 0.98861, 'ap': 0.91381, 'f1': 0.85967, 'precision': 0.90932, 'recall': 0.81516},
    {'model': 'Hybrid GNN + LightGBM (拼接社群特征)', 'auc': 0.98869, 'ap': 0.91273, 'f1': 0.85886, 'precision': 0.91814, 'recall': 0.80677},
]
...
def create_model_layout():
    ...
    return html.Div([
        ...
        # 第二行：增强机制混合和特征重要性（原混淆矩阵位置替换为特征重要性，保持两列布局）
        html.Div([
            html.Div([
            ...
            html.Div([
                ...
                dcc.Graph(id='feature-importance', style={'height': '320px'}, config={'displayModeBar': False})
            ], style={'flex': '1', 'backgroundColor': COLORS['card'], 'padding': '15px', 'borderRadius': '10px'}),
        ], style={'display': 'flex', 'gap': '20px'}),
        # 删除了原来的第三行（特征重要性单独一行），现在特征重要性与增强机制混合并排显示
    ], style={'padding': '20px'})
